...
#####
H15_inter$TRTP1
clinical_arm<- subset(H15_inter, TRTP1 == "Arm 3 (Nintedanib)")
clinical_arm_spe <- clinical_arm
summary(clinical_arm$FGFR1_total)
clinical_arm$PDGFRA_total
clinical_arm$PDGFRB_total


pdf("G:/내 드라이브/작업/프로젝트/HNSCC/수정2/추가수정3/Figure/ARM3_FGFR.pdf")
pdf("G:/내 드라이브/작업/프로젝트/HNSCC/수정2/추가수정3/Figure/Fig_2K.pdf", height = 9, width = 6)
km_AG_fit <- survfit(Surv(OS_TIME1, DEATH) ~ FGFR1_total, data=clinical_arm)
# ggsurvplot(km_AG_fit, title = "ARM3",data =clinical_arm, pval = TRUE,conf.int = TRUE, risk.table = TRUE)
...
ggsurvplot(km_AG_fit, pval = TRUE, 
           ...
           # xscale = "y_m",
           font.legend=12,
           legend.labs = c('Without FGFR mut','With FGFR mut'))  # To display time in years
dev.off()


dependent_os  <- "Surv(OS_TIME1, DEATH)"

explanatory   <- c("final_HPV", "FGFR1_total")
