...
clinical_arms <- H15_inter[H15_inter$TRTP1=="Arm 3 (Nintedanib)",]
...
# nrow(mat_sorted)
sum(mutation_matrix['FGFR1',] != "")
ncol(mat_sorted)

sum(mutation_matrix['FGFR1',] != "")/ncol(mat_sorted)
sum(mutation_matrix['FGFR1',] != "")/419
ncol(mat_sorted)/419
################
ncol(mat_sorted)
ncol(mat_sorted[,mat_sorted['FGFR1',] == 'Amplification'])
22/40
...
# pdf("Figure1_ARM3_2.pdf", width = 35, height = 7)
# draw(fig1, padding = unit(c(2, 2, 10, 2), "mm"), heatmap_legend_list = lgd, heatmap_legend_side = "right")
pdf("/Users/hwangso_macmini/Google Drive/My Drive/Work/project/HNSCC/수정2/추가수정4/Figure/Figure2J.pdf", width = 35, height = 6.5)
draw(fig1_2, padding = unit(c(2, 2, 10, 2), "mm"), heatmap_legend_list = lgd, heatmap_legend_side = "right")
dev.off()
# pdf("/data/project/TRIUMPH/Figure4/figure_Arm/Figure1_ARM3.pdf", width = 35, height = 6.5)
# # draw(fig1, padding = unit(c(2, 2, 10, 2), "mm"), heatmap_legend_list = lgd, heatmap_legend_side = "right")
# draw(fig1_2, padding = unit(c(2, 2, 10, 2), "mm"), heatmap_legend_list = lgd, heatmap_legend_side = "right")
# dev.off()
