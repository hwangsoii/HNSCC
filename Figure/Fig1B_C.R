...
activities <- read.csv("/Users/hwangso_macmini/Google Drive/My Drive/Work/project/HNSCC_2/TRIUMPH_result/COSMIC_SBS96_Activities.txt", sep = "\t", header = TRUE, row.names = 1)
...
my_signatures <- read.csv("/Users/hwangso_macmini/Google Drive/My Drive/Work/project/HNSCC_2/TRIUMPH_result/SBS96_De-Novo_Signatures.txt", sep ='\t')
...
etiology_vector <- c("APOBEC Cytidine Deaminase (C>T)", "Unknown : Clock-like Signature", "Defective DNA mismatch repair", "Thiopurine chemoterapy treatment")
best_fit <- data.frame(
