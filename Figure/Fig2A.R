...
###
# # mat_sorted <- mat_sorted[,1:137]
mat_sorted <- mat_sorted[,1:146]
...
clinical_arms <- H15_inter[H15_inter$TRTP1=="Arm 1 (BYL719)",]
...
data <- All_cnvkit@data
data_PIK3CA <- data[data$Hugo_Symbol=='PIK3CA']
df <- total_clinical
...
lgd <- list(Legend(labels = names(alter_fun)[2:length(alter_fun)], title = "Alteration", labels_gp = gpar(fontsize = 20),  title_gp = gpar(fontsize = 20),
            ...
            Legend(labels = c("Positive", "Negative"), title = "p16", labels_gp = gpar(fontsize = 20), title_gp = gpar(fontsize = 20),
...
pdf("/Users/hwangso_macmini/Google Drive/My Drive/Work/project/HNSCC/수정2/추가수정4/Figure/Figure2A.pdf", width = 35, height = 10)
